...
namespace AvatarsInGraveDanger
{
     ...
     public class ExplosiveEnemy : Enemy
     {
          ...
          public ExplosiveEnemy(Vector3 position, Wave wave)
               : base(PixelEngine.EngineCore.Game, wave)
          {
               ...
               this.Speed *= MathHelper.Clamp(
                    (1.0f + (0.05f * wave.WaveNumber)), 1.0f, 1.75f);
          ...
          public override void Update(GameTime gameTime)
          {
               if (this.IsDying)
               {
                    ...
                    if (addExplosion)
                    {
                         Vector2 explosionPosition = GraphicsHelper.ConvertToScreenspaceVector2(this.WorldPosition);
                         AddExplosion(explosionPosition, 8, 45.0f * 10.0f, 2500.0f, gameTime);// AddExplosion(explosionPosition, 16, 45.0f * 10, 5000.0f, gameTime);
                         addExplosion = false;
                    }
          ...
          {
               foreach (Enemy nearbyEnemy in Wave.GetEnemies())
               {
                    ...
                    if (nearbyEnemy.GetType().Equals(typeof(ExplosiveEnemy)))
                         continue;
