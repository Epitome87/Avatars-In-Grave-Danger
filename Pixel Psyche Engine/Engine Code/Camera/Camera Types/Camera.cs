     ...
     public class Camera : ICameraTrackable
     {
          ...
          public virtual void Reset(Viewport viewport)
          {
               ...
               if (viewport.Width == 1920 && viewport.Height == 1080)
               {
                    ...
                    //MathHelper.ToDegrees(     (float)(2.0f * Math.Atan(MathHelper.ToRadians( (4.0f / 3.0f) / (aspectRatio)) ) * Math.Tan(MathHelper.ToRadians(65.0f) / 2.0f))   ); 
               //2 * atan( (old aspect / new aspect) * tan(old fov / 2) )   (16.0 / 9.0) for 720p

               // MY THEORY:
               // Ratio you want divided by base ratio.
               // Take that and multiply it to base field of view.
               // Example: (16/9) / (4/3) = 1.333333, multiply by 45 degrees = 60
               ...
               projectionMatrix = //PerspectiveFov(fieldOfView, aspectRatio, nearViewPlane, farViewPlane);
                    Matrix.CreatePerspectiveFieldOfView(MathHelper.ToRadians(fieldOfView), aspectRatio, nearViewPlane, farViewPlane);
          ...
          // NEW TESTING FOR RESOLUTION DEPENDANT FIELD OF VIEW
          public static Matrix PerspectiveFov(double fov, double aspect, double near, double far)
          {
               double yScale = 1.0 / Math.Tan(fov / 2);
               double xScale = yScale / aspect;

               return new Matrix((float)xScale, 0, 0, 0, 0, (float)yScale, 0, 0, 0, 0, 
                    (float)(far / (far - near)), 1, 0, 0, - (float)(near * far / (far - near)), 0);
          }
          ...
          public virtual void HandleInput(InputState input, PlayerIndex? ControllingPlayer, PlayerIndex playerIndex)
          {
               ...
               if (input.IsNewButtonPress(Buttons.DPadDown, PixelEngine.EngineCore.ControllingPlayer, out playerIndex))
               {
                    ...
                    CameraManager.ActiveCamera.FieldOfView = 118f;
               }
