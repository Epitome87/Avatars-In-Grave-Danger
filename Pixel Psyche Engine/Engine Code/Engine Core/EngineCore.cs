...
namespace PixelEngine
{
     ...
     {
          public static Rectangle CustomizableSafeArea = new Rectangle(0, 0, 1920, 1080);
          ...
          private static bool resolutionIndependent = false;
          private static Vector3 resolutionScalingFactor = new Vector3(1, 1, 1);

          public static Vector3 ResolutionScalingFactor
          {
               get
               {
                    if (resolutionIndependent)
                    {
                         float horizontalScaling = (float)EngineCore.GraphicsDevice.PresentationParameters.BackBufferWidth / 1280;
                         float verticalScaling = (float)EngineCore.GraphicsDevice.PresentationParameters.BackBufferHeight / 720;

                         return new Vector3(horizontalScaling, verticalScaling, 1);
                    }
          ...
          public static Matrix ResolutionScalingFactorMatrix
          {
               get
               {
                    return Matrix.CreateScale(ResolutionScalingFactor);
               }
          }
          ...
          public static float ResolutionScale
          {
               get   
               {
                    if (EngineCore.GraphicsDevice.Viewport.Width < 1280 && EngineCore.GraphicsDevice.Viewport.Height < 720)
                    {
                         return
                              (EngineCore.GraphicsDevice.Viewport.Width * EngineCore.GraphicsDevice.Viewport.Height) / (1280f * 720f);
                              //(EngineCore.GraphicsDevice.Viewport.Width * 1280f) + (EngineCore.GraphicsDevice.Viewport.Height * 720f) / 2) ;
                    }
                    ...
                    //return graphicsInformation.ScreenWidth / 1280f; 
               } // gi.ScreenWidth is always 1280 currently, so this is equal to 1 always.
                    //{ return ScreenManager.GraphicsDevice.Viewport.Width / 1280f; }  // Technically this is correct for scaling, but why scale from 1280 to 1920? Bigger != better!
                    // { return ScreenManager.GraphicsDevice.Viewport.Width / 1920f; } // Again, works, especially for downscaling, but I don't want 1280->1920 to be upscaled.
                    
               // Should use this one for downscaling to 480p.
               //{ return ScreenManager.GraphicsDevice.Viewport.AspectRatio / (1920f / 1080f); }// (640f / 480f); }// (1280f / 720f); }// (1920f / 1080f); }
               ...
               : base()
          {
               ...
               graphicsDeviceManager.IsFullScreen = false;// graphicsInformation.IsFullScreen;
          ...
          void graphics_PreparingDeviceSettings(object sender, PreparingDeviceSettingsEventArgs e)
          {
               foreach (DisplayMode displayMode in GraphicsAdapter.DefaultAdapter.SupportedDisplayModes)
               {
                    if (displayMode.Width == 640 && displayMode.Height == 480)
                    {
                         ...
                         e.GraphicsDeviceInformation.PresentationParameters.BackBufferHeight = 720;// displayMode.Height;
                         e.GraphicsDeviceInformation.PresentationParameters.BackBufferWidth = 1280;// displayMode.Width;
                    }
               ...
               screenCenter = new Vector2(e.GraphicsDeviceInformation.PresentationParameters.BackBufferWidth / 2,
                    e.GraphicsDeviceInformation.PresentationParameters.BackBufferHeight / 2);
