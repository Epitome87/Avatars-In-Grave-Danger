...
namespace PixelEngine
{
     ...
     public class Counter
     {
               ...
               : this(start, end, text, textEnd, sound)
          {
               ...
               headerTextObject = new TextObject();
               headerTextObject.Text = text;
               headerTextObject.Scale = 0.5f;
               headerTextObject.Color = Color.Gold;
               headerTextObject.FontType = FontType.TitleFont;
               headerTextObject.IsCenter = true;
               headerTextObject.Position = new Vector2(EngineCore.ScreenCenter.X, EngineCore.ScreenCenter.Y * 1.25f);

               numberTextObject = new TextObject();
               numberTextObject.Text = "5";
               numberTextObject.TextEffect = new MoveInEffect(0.75f, numberTextObject.Text,
                         new Vector2(-10f, EngineCore.ScreenCenter.Y * 1.4f), new Vector2(EngineCore.ScreenCenter.X, EngineCore.ScreenCenter.Y * 1.4f));
               numberTextObject.Scale = 1.0f;
               numberTextObject.Color = Color.Gold;
               numberTextObject.FontType = FontType.TitleFont;
               numberTextObject.IsCenter = true;
          }
          ...
          public void DisplayCountdown(GameTime gameTime)
          {
               ...
               if (currentCount > countdownEnd)
               {
                    if (Math.Abs(currentCount - previousDisplayTime) >= displayInterval)
                    {
                         ...
                         numberTextObject.Update(gameTime);
                         numberTextObject.Draw(gameTime);
               ...
               else
               {
                    TextManager.DrawAutoCentered(PixelEngine.Text.TextManager.TitleFont,
                         countdownTextEnd, new Vector2(EngineCore.ScreenCenter.X, EngineCore.ScreenCenter.Y * 1.3f), Color.CornflowerBlue, 1.0f);
               }
                    ...
                    (currentCount != countdownStart && currentCount != countdownEnd - 1))
               {
                    ...
                    numberTextObject.TextEffect = new MoveInEffect(0.75f, currentCount.ToString(),
                         new Vector2(-10f, EngineCore.ScreenCenter.Y * 1.4f), new Vector2(EngineCore.ScreenCenter.X, EngineCore.ScreenCenter.Y * 1.4f));

                    headerTextObject.Scale += 0.15f;
               }
